...
- **Per-client skille (odizolowane biblioteki)** — Główna biblioteka `C:/Users/getrk/patterns/` zbiera wzorce z projektów typu "nauka kodu" (np. PaniGinekolog — strony powtarzalne między projektami w obrębie własnego portfolio). Dla projektów klienckich takich jak **Live-Food (SpecialSpace)**, **HCBio**, **InfiniteServices** wzorce nie powinny trafiać do głównej biblioteki — każdy taki klient powinien mieć własny, odizolowany skill na bazie `save-pattern` z własną lokalizacją biblioteki (np. `C:/Users/getrk/patterns-specialspace/`). Wariant do implementacji później: sparametryzowany `save-pattern` przyjmujący prefix biblioteki, albo fork skill'a per klient.
